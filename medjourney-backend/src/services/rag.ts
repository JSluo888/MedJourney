...
class MockRAGService implements RAGService {
  ...
  async initialize(): Promise<void> {
    try {
      logger.info('初始化模拟RAG服务...');
      
      // 在实际环境中，这里会使用真实的OpenAI API
      // 模拟环境中，我们使用内存向量存储
      this.embeddings = new OpenAIEmbeddings({
        openAIApiKey: config.ai.openai_api_key === 'mock-openai-key' ? 'mock' : config.ai.openai_api_key
      });
      
      this.llm = new ChatOpenAI({
        openAIApiKey: config.ai.openai_api_key === 'mock-openai-key' ? 'mock' : config.ai.openai_api_key,
        modelName: config.ai.openai_model,
      ...
      logger.info('模拟RAG服务初始化完成');
    } catch (error) {
      logger.error('RAG服务初始化失败', error as Error);
      
      // 如果真实服务失败，使用完全模拟的实现
      this.isInitialized = true;
      logger.warn('使用完全模拟的RAG实现');
    }
  }
  ...
  async addDocuments(documents: Array<{ content: string; metadata: Record<string, any> }>): Promise<void> {
    try {
      ...
      if (this.vectorStore) {
        ...
        logger.info(`添加了 ${docs.length} 个文档到向量库`);
      } else {
        logger.warn('向量存储未初始化，跳过文档添加');
      }
    } catch (error) {
      logger.error('添加文档到向量库失败', error as Error);
      throw new AIServiceError('无法添加文档到知识库');
    }
  }
  ...
  async query(question: string, options?: {
    ...
    if (!this.isInitialized) {
      throw new AIServiceError('RAG服务未初始化');
    }

    try {
      logger.debug('RAG查询开始', { question: question.substring(0, 100) });
      ...
      if (this.chain && this.vectorStore) {
        ...
        return {
          ...
          sources: relevantDocs.map((doc: Document, index: number) => ({
      ...
      logger.error('RAG查询失败', error as Error);
  ...
  private async createRAGChain(): Promise<void> {
    try {
...
请提供温暖、专业且实用的回答：`);
      ...
      logger.error('创建RAG链失败', error as Error);
      throw error;
    }
  }

  private mockQuery(question: string, options?: {
    topK?: number;
    threshold?: number;
    filter?: Record<string, any>;
  }): {
    ...
    logger.debug('使用模拟RAG查询', { question: question.substring(0, 100) });
  ...
  private generateMockAnswer(question: string, relevantDocs: any[]): string {
    const questionLower = question.toLowerCase();
