...
class StepfunAIServiceImpl implements StepfunAIService {
  ...
  constructor() {
    ...
    this.client = new OpenAI({
      apiKey: config.ai.stepfun_api_key,
      baseURL: config.ai.stepfun_base_url,
    });
    this.model = config.ai.stepfun_model;
    this.temperature = config.ai.temperature;
    
    logger.info('Stepfun AI 服务初始化完成', {
      model: this.model,
      baseURL: config.ai.stepfun_base_url
    });
  }
  ...
  async generateResponse(prompt: string, context?: any): Promise<{
    ...
    try {
      ...
      logger.error('Stepfun API调用失败', error, {
        model: this.model,
        promptLength: prompt.length,
      ...
      throw new AIServiceError(
        `Stepfun AI服务调用失败: ${error.message}`,
        'STEPFUN_API_ERROR'
      );
    }
  }

  async *streamResponse(prompt: string, context?: any): AsyncIterableIterator<{
    delta: string;
    done: boolean;
  }> {
    try {
      ...
      for await (const chunk of stream) {
        ...
        if (done) {
          logger.debug('Stepfun流式响应完成');
          break;
        }
      }
    } catch (error: any) {
      logger.error('Stepfun流式API调用失败', error);
      throw new AIServiceError(
        `Stepfun流式服务调用失败: ${error.message}`,
        'STEPFUN_STREAM_ERROR'
      );
    }
  }
...
请返回以下格式的JSON：
{
  ...
  "emotions": {
    ...
    "neutral": 0.0
      ...
      try {
        ...
        logger.warn('情感分析JSON解析失败，使用基于关键词的分析', {
      ...
      logger.error('Stepfun情感分析失败', error);
      return this.fallbackEmotionAnalysis(text);
    }
  }
  ...
  private buildMessages(prompt: string, context?: any): any[] {
    ...
    if (context?.recentMessages && Array.isArray(context.recentMessages)) {
      context.recentMessages.slice(-5).forEach((msg: any) => {
        messages.push({
...
请根据会话类型调整您的响应方式。`;
  ...
  private fallbackEmotionAnalysis(text: string): {
    emotion: string;
    confidence: number;
    emotions: Record<string, number>;
  } {
    ...
    return {
      ...
      confidence: Math.max(emotions[primaryEmotion as keyof typeof emotions], 0.5),
      emotions
    };
  }
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      logger.error('Stepfun健康检查失败', error as Error);
      return false;
    }
  }
  ...
  getStats(): any {
    return {
      ...
      model: this.model,
      baseURL: config.ai.stepfun_base_url,
      isConfigured: !!config.ai.stepfun_api_key
    };
  }
}
...
class StepfunServiceFactory {
  private static instance: StepfunAIService | null = null;
  ...
  static create(): StepfunAIService {
    ...
    logger.info('Stepfun AI服务实例创建完成');
