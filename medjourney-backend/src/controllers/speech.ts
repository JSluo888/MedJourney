...
export class SpeechController {
  ...
  // ElevenLabs 语音合成
  async synthesizeSpeech(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
  ...
  // 流式语音合成
  async streamSpeech(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { text, voice_id, model_id } = req.body;
      const userId = req.user?.patient_id;
      
      logger.info('开始流式语音合成', {
        textLength: text.length,
        voice_id,
        model_id,
        userId
      ...
      try {
        ...
        logger.error('流式语音合成错误', streamError);
      ...
      logger.error('流式语音合成初始化失败', error);
      
      if (!res.headersSent) {
        const response: ApiResponse = ResponseHelper.error(res, 
          '流式语音合成初始化失败',
          'STREAM_SPEECH_INIT_ERROR',
          error.message
        );
        res.status(500).json(response);
      }
  ...
  async getVoices(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      logger.debug('获取ElevenLabs声音列表');
      
      const voices = await this.elevenLabsService.getVoices();
      
      // 过滤出支持中文的声音
      const chineseVoices = voices.filter((voice: any) => 
        voice.language.includes('中文') || 
  ...
  async getStatus(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const response: ApiResponse = ResponseHelper.success(res, status, 'ElevenLabs服务状态获取成功');
      res.status(200).json(response);
    } catch (error: any) {
      logger.error('获取ElevenLabs服务状态失败', error);
      const response: ApiResponse = ResponseHelper.error(res, 
        '获取服务状态失败',
        'GET_STATUS_ERROR',
        error.message
