...
export class AssessmentController {
  ...
  // 分级问诊分析
  async analyzeAssessment(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { sessionId, assessmentType, includeRecommendations } = req.body;
      const userId = req.user?.patient_id;
      
      logger.info('开始分级问诊分析', {
        sessionId,
        assessmentType,
      ...
      logger.info('分级问诊分析完成', {
        sessionId,
        assessmentType,
        analysisKeys: Object.keys(analysisResult)
      });
      
      const response: ApiResponse = ResponseHelper.success(res, analysisResult, '分级问诊分析完成');
      res.status(200).json(response);
    } catch (error: any) {
      logger.error('分级问诊分析失败', error);
      const response: ApiResponse = ResponseHelper.error(res, 
        '分级问诊分析失败',
        'ASSESSMENT_ANALYSIS_ERROR',
        error.message
      );
  ...
  // 提交问诊数据
  async submitAssessmentData(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
  ...
  // 获取问诊历史
  async getAssessmentHistory(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const response: ApiResponse = ResponseHelper.success(res, history, '问诊历史获取成功');
      res.status(200).json(response);
    } catch (error: any) {
      logger.error('获取问诊历史失败', error);
      const response: ApiResponse = ResponseHelper.error(res, 
        '获取问诊历史失败',
        'ASSESSMENT_HISTORY_ERROR',
        error.message
      );
      res.status(500).json(response);
    }
  ...
  // 获取问诊报告
  async getAssessmentReport(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      if (format === 'pdf') {
        ...
        const response: ApiResponse = ResponseHelper.success(res, fullReport, '问诊报告生成成功');
        res.status(200).json(response);
      }
    } catch (error: any) {
      logger.error('生成问诊报告失败', error);
      const response: ApiResponse = ResponseHelper.error(res, 
        '生成问诊报告失败',
        'ASSESSMENT_REPORT_ERROR',
        error.message
      );
      res.status(500).json(response);
    }
  ...
  private async processAssessmentData(data: any, assessmentType: string): Promise<any> {
    // 数据清理和验证
  ...
  private async saveAssessmentData(assessmentData: any): Promise<any> {
  ...
  private async saveAssessmentAnalysis(sessionId: string, analysis: any): Promise<void> {
    const query = `
  ...
  private async generatePreliminaryAnalysis(data: any, assessmentType: string): Promise<any> {
    // 生成初步分析结果
    const analysis = {
      assessment_type: assessmentType,
  ...
  private async getAssessmentHistoryByPatient(patientId: string, options: any): Promise<any> {
  ...
  private async getAssessmentDataBySession(sessionId: string): Promise<any[]> {
  ...
  // 评估计算方法
  private assessMemoryPerformance(cognitiveIndicators: any): any {
    return {
  ...
  private assessAttentionPerformance(cognitiveIndicators: any): any {
    return {
  ...
  private assessLanguagePerformance(cognitiveIndicators: any): any {
    return {
  ...
  private assessMoodStability(emotionalSummary: any): any {
    return {
  ...
  private identifyEmotionalPatterns(emotionalSummary: any): string[] {
    const patterns = [];
  ...
  private calculateOverallAssessmentScore(analysis: any): number {
  ...
  private assessCognitiveRisk(analysis: any): string {
  ...
  private categorizeSocialEngagement(socialEngagement: any): string {
    const score = socialEngagement.initiative_score;
    
    if (score > 0.7) return '高度参与';
    if (score > 0.4) return '中等参与';
    return '低度参与';
  }

  private generateKeyObservations(analysis: any): string[] {
    const observations = [];
  ...
  private generateNextSteps(analysis: any, assessmentType: string): string[] {
    const nextSteps = [];
  ...
  private recommendConversationType(analysis: any): string {
  ...
  private estimateConversationDuration(analysis: any): number {
    // 根据分析结果估算对话时长（分钟）
    const baseTime = 15;
  ...
  // 辅助方法
  private calculateBasicInfoCompleteness(data: any): number {
  ...
  private calculateCognitiveTestScore(data: any): number {
    // 模拟认知测试计分
    return 0.7 + Math.random() * 0.2;
  }

  private calculateEmotionalStateScore(data: any): number {
    // 模拟情绪状态计分
  ...
  private identifyRiskIndicators(data: any): string[] {
    const indicators = [];
  ...
  private categorizeStability(score: number): string {
    if (score > 0.7) return '稳定';
    if (score > 0.4) return '较稳定';
    return '不稳定';
  }

  private generateMemoryNotes(score: number): string {
    if (score > 0.7) return '记忆功能表现良好，继续保持';
    if (score > 0.4) return '记忆功能有轻度下降，建议增加记忆训练';
    return '记忆功能明显受损，需要专业干预';
  }

  private generateAttentionNotes(score: number): string {
    if (score > 0.7) return '注意力集中度良好';
    if (score > 0.4) return '注意力集中有些困难，建议注意力训练';
    return '注意力集中困难显著，需要专业评估';
  }

  private generateLanguageNotes(score: number): string {
    if (score > 0.7) return '语言表达能力正常';
    if (score > 0.4) return '语言表达有轻度影响，需要更多耐心';
