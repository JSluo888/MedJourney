...
const ten_framework_1 = __importDefault(require("../services/ten-framework"));
...
class ConversationController {
    ...
    constructor() {
        this.tenService = ten_framework_1.default.create();
    ...
    async startConversation(req, res) {
        try {
            ...
            logger_1.logger.info('启动TEN Framework对话会话', {
                patientId,
                sessionType,
                userId
            });
            ...
            const response = (0, response_1.successResponse)({
            ...
            const response = (0, response_1.errorResponse)('启动对话会话失败', 'CONVERSATION_START_ERROR', error.message);
    ...
    async sendMessage(req, res) {
        try {
            ...
            if (!session) {
                const response = (0, response_1.errorResponse)('会话不存在', 'SESSION_NOT_FOUND');
            ...
            const response = (0, response_1.errorResponse)('发送消息失败', 'MESSAGE_SEND_ERROR', error.message);
            res.status(500).json(response);
        }
    }
    // 获取实时对话分析
    async getAnalysis(req, res) {
        try {
            ...
            const response = (0, response_1.successResponse)(result, '对话分析获取成功');
            res.status(200).json(response);
        }
        catch (error) {
            logger_1.logger.error('获取对话分析失败', error);
            const response = (0, response_1.errorResponse)('获取对话分析失败', 'ANALYSIS_GET_ERROR', error.message);
            res.status(500).json(response);
        }
    }
    // 获取对话消息历史
    async getMessages(req, res) {
        try {
            ...
            const response = (0, response_1.successResponse)(messages, '消息历史获取成功');
            res.status(200).json(response);
        }
        catch (error) {
            logger_1.logger.error('获取消息历史失败', error);
            const response = (0, response_1.errorResponse)('获取消息历史失败', 'MESSAGES_GET_ERROR', error.message);
            res.status(500).json(response);
        }
    }
    // 获取会话状态
    async getStatus(req, res) {
        try {
            ...
            const response = (0, response_1.successResponse)(status, '会话状态获取成功');
            res.status(200).json(response);
        }
        catch (error) {
            logger_1.logger.error('获取会话状态失败', error);
            const response = (0, response_1.errorResponse)('获取会话状态失败', 'STATUS_GET_ERROR', error.message);
            res.status(500).json(response);
        }
    }
    // 结束对话会话
    async endConversation(req, res) {
        try {
            ...
            logger_1.logger.info('对话会话已结束', { sessionId });
            const response = (0, response_1.successResponse)({
                sessionId,
                endedAt: new Date()
            }, '对话会话已结束');
            ...
            const response = (0, response_1.errorResponse)('结束对话会话失败', 'CONVERSATION_END_ERROR', error.message);
            res.status(500).json(response);
        }
    }
    // 处理 WebSocket 连接
    async handleWebSocket(req, res) {
        try {
            ...
            logger_1.logger.info('WebSocket 连接请求', { sessionId });
            const response = (0, response_1.successResponse)({
            ...
            logger_1.logger.error('WebSocket 连接失败', error);
            const response = (0, response_1.errorResponse)('WebSocket 连接失败', 'WEBSOCKET_CONNECTION_ERROR', error.message);
            res.status(500).json(response);
        }
    }
    // 私有方法
    async initializeTENFramework() {
        try {
            await this.tenService.connect();
            // 设置消息监听器
            this.tenService.onMessage((message) => {
                this.handleTENMessage(message);
            });
            // 设置状态监听器
            this.tenService.onStatusChange((status) => {
                this.handleTENStatusChange(status);
            });
            logger_1.logger.info('TEN Framework 初始化完成');
        }
        catch (error) {
            logger_1.logger.error('TEN Framework 初始化失败', error);
        }
    }
    async handleTENMessage(message) {
        try {
            logger_1.logger.debug('接收到 TEN Framework 消息', {
            ...
            logger_1.logger.error('处理 TEN Framework 消息失败', error);
        }
    }
    async handleTENStatusChange(status) {
        try {
            logger_1.logger.debug('TEN Framework 状态变化', status);
            // 通过 WebSocket 转发状态更新
            ...
            logger_1.logger.error('处理 TEN Framework 状态变化失败', error);
        }
    }
    broadcastToWebSocket(tenSessionId, message) {
        // 这里应该根据 TEN 会话 ID 找到对应的 WebSocket 连接
        // 并发送消息给前端
        logger_1.logger.debug('WebSocket 广播消息', {
            tenSessionId,
            messageType: message.type
        });
    }
    async createSession(patientId, sessionType) {
    ...
    async getMessagesBySessionId(sessionId, options) {
    ...
    async getSessionIdByTenSessionId(tenSessionId) {
...
//# sourceMappingURL=conversation-old.js.map