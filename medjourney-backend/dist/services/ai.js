...
exports.OpenAIService = exports.MockAIService = exports.AIServiceFactory = void 0;
...
class MockAIService {
    ...
    async generateResponse(prompt, context) {
        if (!this.isAvailable) {
            throw new errors_1.AIServiceError('AI服务不可用');
        }
        // 模拟处理延迟
        const processingTime = 800 + Math.random() * 1200;
        await new Promise(resolve => setTimeout(resolve, processingTime));
        logger_1.logger.ai('generateResponse', 'mock-gpt-4', 0, processingTime, { prompt: prompt.substring(0, 100) });
        // 根据输入生成智能回复
    ...
    async analyzeEmotion(text) {
        if (!this.isAvailable) {
            throw new errors_1.AIServiceError('情感分析服务不可用');
        }
        // 模拟处理延迟
    ...
    async extractEntities(text) {
        if (!this.isAvailable) {
            throw new errors_1.AIServiceError('实体提取服务不可用');
        }
        // 模拟处理延迟
        await new Promise(resolve => setTimeout(resolve, 300 + Math.random() * 500));
        const entities = this.extractTextEntities(text);
        logger_1.logger.debug('模拟实体提取', {
            text: text.substring(0, 50),
    ...
    setAvailability(available) {
        ...
        logger_1.logger.info(`模拟AI服务可用性设置为: ${available}`);
    }
}
...
class OpenAIService {
    ...
    temperature;
    constructor() {
        this.apiKey = config_1.config.ai.openai_api_key;
        this.model = config_1.config.ai.openai_model;
    ...
    async generateResponse(prompt, context) {
        try {
            ...
            logger_1.logger.ai('generateResponse', this.model, response.data.usage.total_tokens, processingTime, {
            ...
            logger_1.logger.error('OpenAI API 调用失败', error);
            throw new errors_1.AIServiceError('OpenAI 服务调用失败', 'OpenAI');
        }
    }
    async analyzeEmotion(text) {
        try {
            ...
            logger_1.logger.error('OpenAI 情感分析失败', error);
            throw new errors_1.AIServiceError('情感分析失败', 'OpenAI');
        }
    }
    async extractEntities(text) {
        try {
            ...
            logger_1.logger.error('OpenAI 实体提取失败', error);
            throw new errors_1.AIServiceError('实体提取失败', 'OpenAI');
        }
    }
...
class AIServiceFactory {
    ...
    static create() {
        ...
        let service;
        // 在生产环境中优先使用 OpenAI
        if (config_1.config.server.env === 'production' && config_1.config.ai.openai_api_key !== 'mock-openai-key') {
            try {
                service = new OpenAIService();
                logger_1.logger.info('使用 OpenAI 服务');
            }
            catch (error) {
                logger_1.logger.warn('OpenAI 初始化失败，使用模拟服务', { error });
                service = new MockAIService();
            }
        }
        else {
            logger_1.logger.info('使用模拟AI服务');
            service = new MockAIService();
        }
