...
class EnhancedAgoraService {
  ...
  private async initializeTENService(): Promise<void> {
    try {
      // TEN Framework配置
      const tenConfig = {
        websocketUrl: 'ws://localhost:8080/ten-agent', // 实际应用中的TEN Agent地址
        language: 'zh-CN',
        voiceId: 'zh-CN-XiaoxiaoNeural' // Azure TTS语音ID
      };
  ...
  private async startTENSession(): Promise<void> {
    if (this.tenService) {
      try {
        await this.tenService.startSession(this.uid);
    ...
    try {
      ...
      this.mediaRecorder.onstop = async () => {
        if (this.audioChunks.length > 0) {
          ...
          if (this.tenService) {
            try {
              await this.tenService.sendAudioData(audioBuffer);
            } catch (error) {
  ...
  async sendImageMessage(imageUrl: string): Promise<void> {
    ...
    if (this.tenService) {
      try {
        await this.tenService.sendImageData(imageUrl);
      } catch (error) {
  ...
  async disconnect(): Promise<void> {
    try {
      ...
      // 断开TEN Framework连接
      if (this.tenService) {
        await this.tenService.endSession();
  ...
  getTENConnectionState(): boolean {
    return this.tenService?.getConnectionState() || false;
  }
